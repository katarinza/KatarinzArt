...
class PencilDrawing():
    ...
    def __init__(self, file):
        self.file = file # file - input image
        self.size = 11 # size of drawn lines
        self.w1 = 11
        self.w2 = 37
        self.w3 = 52
        self.sigma_b = 9
    ...
    def draw(self):
        """Main function."""
        # Read file and save returned ndarray in self.so.
        self.so = skimage.data.imread(self.file)
        self.source = skimage.data.imread(self.file, as_grey = True)
        self.vzorec = skimage.data.imread('vzorec.jpg', as_grey = True)
        scipy.misc.imsave("source.jpg", self.source)
        # Compute the luminance of an image.
        #self.source = 0.3*self.so[:,:,0] + 0.59*self.so[:,:,1] + 0.11*self.so[:,:,2]
...
#############################################################################
    def sketch(self):
        self.gradient = numpy.gradient(self.source) # central differences
    ...
    def set_classification(self):
        ...
        for key, G in self.response_maps.iteritems():
            ...
            scipy.misc.imsave("neki%d.jpg"%i, C)
            i = i + 1
    ...
    def set_response_maps(self):
        ...
        for key, K in self.kernels.iteritems():
            ...
            scipy.misc.imsave("hopsasa%d.jpg"%i, G)
            i = i + 1
